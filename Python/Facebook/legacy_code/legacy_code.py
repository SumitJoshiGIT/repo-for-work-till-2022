          ...
          async with aiohttp.ClientSession() as session:
             ...
             for agent in agents:
               ...
               if self.bot:                 
                 ...
                 try:[await self.SendText(message,user["sample"],session) async for user in self.Users.find()]
    ...
    async def register_account(self, browser,human,captcha_,session,UA):
      ...
      async def inputs(selector, input_=None, selection="click"):
        ...
        if selection == "keyboard":
            await page.keyboard.type(input_,delay=uniform(15,19))
        ...
        if sub1==True :
          ...
          try:
           while  await page.is_visible(selector): 
                if n==0:
                     if await page.is_visible('a[role="button"]:has-text("No, Create New Account")'):
                      return False      
                elif n>0:
      ...
      async def captcha_solve(check=1):
       ...
       # if await page.locator('span:has-text("We need more information")').is_visible():
            ...
            try:    
             ...
             await inputs('input[name="contactpoint"]',mynumber.current_number,"keyboard")
